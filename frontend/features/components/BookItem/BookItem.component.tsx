...
export const BookItem: FunctionComponent<BookItemProps> = ({
  ...
  const [loanProcesss, setLoanProcess] = useState<{
    isLoaning: boolean
    message: string
  }>({ isLoaning: false, message: "Aguarde" })
  ...
  const handleLoanBookAction = async (bookId: string) => {
    ...
    try {
      ...
      setLoanProcess({
        isLoaning: true,
        message: "Occorreu Um Erro 😔. Talvez já tenha sido emprestado",
      })
    }
    setTimeout(
      () => setLoanProcess({ isLoaning: false, message: "Aguarde" }),
      1500
    )
  }
  return (
    <Card variant="outlined">
      ...
      <CardContent>
        ...
        {loanProcesss.isLoaning && (
          <Typography variant="body2">{loanProcesss.message}</Typography>
        )}
      </CardContent>
      <CardActions>
        <Button
          ...
          disabled={loanProcesss.isLoaning}
        >
          Empréstimo
        </Button>
      </CardActions>
